token=$(curl https://auth.docker.io/token?service=registry.docker.io\&scope=repository:peer2profit/peer2profit_linux:pull | python3 -c 'import json,sys,builtins; builtins.print(json.loads(sys.stdin.read()).get("token"))')
